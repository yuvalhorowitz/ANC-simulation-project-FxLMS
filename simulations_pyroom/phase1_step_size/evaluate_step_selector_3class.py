"""
Evaluate 3-Class Step Size Selector on Phase 1 Criteria

Tests the 3-class model (Low/Medium/High μ) against baseline fixed μ=0.005
...
def simulate_anc(scenario: str, variation: int, step_size: float, use_adaptive: bool = False, model=None):
    ...
    Args:
        ...
        step_size: fixed step size (if not adaptive) OR initial step size
        use_adaptive: if True, use model to select step size
        model: 3-class model (required if use_adaptive=True)

    Returns:
        dict with results
    """
    # Generate noise
    mixer = NoiseMixer(fs=FS)
    noise_signal = mixer.generate_scenario(scenario, duration=DURATION, seed=variation)
    ...
    if use_adaptive and model is not None:
        ...
        # Map class to step size range midpoint
        # Class 0 (low): use 0.004 (midpoint of 0.003-0.005)
        # Class 1 (medium): use 0.0085 (midpoint of 0.007-0.010)
        # Class 2 (high): use 0.015
        CLASS_TO_STEP_SIZE = {
            0: 0.004,   # Low μ
            1: 0.0085,  # Medium μ
            2: 0.015    # High μ
...
def main():
    print("=" * 70)
    print("Phase 1 Evaluation: 3-Class Step Size Selector vs Baseline")
    print("=" * 70)

    # Load 3-class model
    model_path = Path('output/models/phase1/step_selector_3class.pt')
    print(f"\nLoading 3-class model from: {model_path}")
    model = ThreeClassStepSizeSelector.load(model_path)
    ...
    # Save results
    output_path = Path('output/data/phase1/evaluation_results_3class.json')
    ...
    if phase1_passed:
        ...
        print("The 3-class model successfully adapts μ to different scenarios.")
        print("=" * 70)
    else:
        print("\n" + "=" * 70)
